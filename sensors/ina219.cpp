/**
 * This is the main file for the INA219 Sensor. The sensor reads power from the Waveshare 3S UPS.
 * 
*/
...
void INA219::wireWriteRegister(uint8_t reg, uint16_t value) {
  uint8_t values[2] = {(uint8_t)1, (uint8_t)1};
  values[0] = value & 0xFF;
  values[1] = (value & 0xFF00) >> 8;
	wiringPiI2CWriteBlockData(fd, reg, values, 2);
  printf("Writing %d to reg = %d and fd = %d\n", value, reg, fd);
  // wiringPiI2CWriteReg8(fd, reg, (value >> 8) & 0xFF);
  // wiringPiI2CWriteReg8(fd, reg, value & 0xFF);
}
...
void INA219::wireReadRegister(uint8_t reg, uint16_t *value) {
  ...
  uint8_t values[2] = {(uint8_t)1, (uint8_t)1}; 
  int status = wiringPiI2CReadBlockData(fd, reg, values, 2);

  // int status = wiringPiI2CReadReg16(fd, reg);
  // int status2a = wiringPiI2CReadReg8(fd, reg);  
  // int status2b = wiringPiI2CReadReg8(fd, reg);
  // int status2 = status2a * 256 +  status2b;
  *value = values[0] * 256 + values[1];
	printf("The value is [%d, %d] = %d with status = %d for fd = %d\n", values[0], values[1], *value, status, fd);
 ...
 */
INA219::INA219(const char *device, uint8_t addr) {
	ina219_device = device;
  ina219_i2caddr = addr;
 ...
 */
void INA219::init() {
	fd = wiringPiI2CSetupInterface(ina219_device, ina219_i2caddr);
	printf("Intializing %s at %d", ina219_device, ina219_i2caddr);
	if (fd == -1) {
		throw std::runtime_error("Failed to init INA219");
	}
	printf("Fd is %d\n", fd);
